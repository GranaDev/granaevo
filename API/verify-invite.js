...
function checkRate(ip) {
  const now = Date.now()
  const r   = RATE_STORE.get(ip)
  if (!r || now - r.t > RATE_WINDOW_MS) { RATE_STORE.set(ip, { c: 1, t: now }); return true }
...
export default async function handler(req, res) {
  ...
  if (!EDGE_URL || !ANON_KEY) {
    console.error('[verify-invite] Env vars ausentes')
  ...
  try {
    ...
    console.error('[verify-invite] upstream error:', e.message)
