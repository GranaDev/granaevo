...
function render() {
    ...
    const saidas   = itens.filter(i => i.tipo === 'saida').reduce((s, i) => s + i.valor, 0);
    ...
    if (itens.length === 0) {
        ...
        for (const item of [...itens].reverse()) {
            ...
            val.className = 'trial-item-val' + (item.tipo === 'saida' ? ' trial-neg' : ' trial-pos');
            val.textContent = (item.tipo === 'saida' ? '− ' : '+ ') + BRL.format(item.valor);
