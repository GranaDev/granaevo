...
// Carrega o sub-chunk de exportação sob demanda (só no clique) e chama a função.
// As 4 funções de export vivem em db-relatorios-export.js (fora do chunk de boot).
// _ctx + os helpers compartilhados com o render são injetados via init.
async function _lazyExport(nome) {
    try {
        const m = await import('./db-relatorios-export.js');
        m.init(_ctx, { _escapeXml, _getPeriodInfo, _getTxsDoPeriodo, _relExpandirTx, _downloadBlob });
        return m[nome]();
    } catch {
        _ctx.mostrarNotificacao('Não foi possível carregar a exportação. Tente de novo.', 'error');
    }
}
    ...
    _ctx.criarPopupDOM((box) => {
        ...
        const opts = [
            { icon:'fa-file-pdf',       color:'#ef4444', label:'PDF',                badge:'Recomendado',
              ...
              fn: () => { _ctx.fecharPopup(); setTimeout(() => _lazyExport('_exportPDF'), 120); } },
            { icon:'fa-file-csv',       color:'#0891b2', label:'CSV',
              ...
              fn: () => { _ctx.fecharPopup(); setTimeout(() => _lazyExport('_exportCSV'), 120); } },
            { icon:'fa-file-excel',     color:'#16a34a', label:'Excel (.xls)',
              desc:'Planilha com 4 abas: Resumo, Transações, Metas e Contas Fixas.',
              fn: () => { _ctx.fecharPopup(); setTimeout(() => _lazyExport('_exportExcel'), 120); } },
            { icon:'fa-file-powerpoint',color:'#ea580c', label:'Apresentação (.html)',
              ...
              fn: () => { _ctx.fecharPopup(); setTimeout(() => _lazyExport('_exportApresentacao'), 120); } },
        ];
